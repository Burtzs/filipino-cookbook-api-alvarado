...
// ============================================================================
// Load Configuration (credentials kept out of version control)
// ============================================================================
...
if (!file_exists($configPath)) {
    die(json_encode([
        'status' => 'error',
        'message' => 'Configuration file not found. Copy config.example.php to config.php and set your local values.'
    ]));
}

require $configPath; // defines $dbHost, $dbName, $dbUser, $dbPass, $apiToken
...
try {
    ...
    // Secure error handling: do not leak connection details or driver messages
...
$tokenMiddleware = function (Request $request, $handler) use ($apiToken) {
    $authHeader = $request->getHeaderLine('Authorization');

    // Check if Authorization header is present and has correct Bearer token
    if (empty($authHeader) || $authHeader !== 'Bearer ' . $apiToken) {
...
$app->group('/api', function (RouteCollectorProxy $group) use ($pdo) {
    ...
    $group->post('/foods', function (Request $request, Response $response) use ($pdo) {
        ...
        try {
            ...
            $pdo->rollBack();
            error_log('Failed to add food: ' . $e->getMessage());
            return jsonResponse($response, [
                'status' => 'error',
    ...
    $group->get('/categories/{id}/foods', function (Request $request, Response $response, array $args) use ($pdo) {
        ...
        $catStmt = $pdo->prepare("SELECT category_id FROM categories WHERE category_id = :id");
    ...
    // ========================================================================
    // 10. Get Foods by Origin
    // GET /api/origins/{id}/foods
    // (Optional Enhancement - Option A)
    // ========================================================================

    $group->get('/origins/{id}/foods', function (Request $request, Response $response, array $args) use ($pdo) {
        if (!ctype_digit($args['id'])) {
            return jsonResponse($response, [
                'status' => 'error',
        ...
        $originStmt = $pdo->prepare("SELECT origin_id FROM origins WHERE origin_id = :id");
